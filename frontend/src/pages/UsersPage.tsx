...
import { Plus, Search, ToggleLeft, ToggleRight, KeyRound, Eye, EyeOff, Trash2, Mail } from 'lucide-react';
...
export default function UsersPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowCreate(false);
      setForm({ name: '', email: '', role: 'sales', department: '', phone: '', baseSalary: '' });
      load();
    } catch (err: unknown) {
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Add New User">
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="flex items-start gap-2 bg-violet-50 border border-violet-200 rounded-lg px-4 py-3 text-sm text-violet-700">
            ...
            <span>An invite email will be sent to the user with a link to set their own password.</span>
          </div>
