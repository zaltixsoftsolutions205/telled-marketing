...
import { Plus, Search, ToggleLeft, ToggleRight, KeyRound } from 'lucide-react';
...
export default function UsersPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', role: 'sales' as Role, phone: '' });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowCreate(true)} className="btn-primary flex items-center gap-2"><Plus size={16} /> Add User</button>
      </div>
                ...
                {users.map((u) => (
                  <tr key={u._id} className="hover:bg-violet-50/20 transition-colors">
                    ...
                    <td className="table-cell">
                      <span className="badge bg-violet-100 text-violet-800 capitalize">{u.role.replace('_', ' ')}</span>
                    </td>
                    ...
                    <td className="table-cell">
                      <div className="flex items-center gap-2">
                        <button onClick={() => setToggleTarget(u)} title={u.isActive ? 'Deactivate' : 'Activate'} className="p-1 text-gray-400 hover:text-violet-600">
                          {u.isActive ? <ToggleRight size={18} className="text-green-500" /> : <ToggleLeft size={18} />}
                        </button>
                        <button onClick={() => { setSelected(u); setShowReset(true); }} title="Reset Password" className="p-1 text-gray-400 hover:text-violet-600">
                          <KeyRound size={15} />
                        </button>
                      </div>
                    </td>
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input required type="password" className="input-field" value={form.password} onChange={(e) => setForm(f => ({...f, password: e.target.value}))} placeholder="Min 8 characters" minLength={8} />
            </div>
            <div>
              <label className="label">Role *</label>
              <select required className="input-field" value={form.role} onChange={(e) => setForm(f => ({...f, role: e.target.value as Role}))}>
      ...
      <Modal isOpen={showReset} onClose={() => setShowReset(false)} title={`Reset Password — ${selected?.name}`} size="sm">
        <form onSubmit={handleReset} className="space-y-4">
          <div>
            ...
            <input required type="password" className="input-field" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} minLength={8} placeholder="Min 8 characters" />
          </div>
          <div className="flex gap-3 justify-end">
