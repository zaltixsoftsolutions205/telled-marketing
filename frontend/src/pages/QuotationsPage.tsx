...
import { 
  ...
  Percent, Building2, RefreshCw, Eye, Trash2, Edit
} from 'lucide-react';
...
export default function QuotationsPage() {
  ...
  const openVendorModal = (quotation: Quotation) => {
    setSelectedQuotation(quotation);
    setVendorForm({
      vendorEmail: (quotation.leadId as any)?.oemEmail || '',
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.leadId) {
      setToast({ message: 'Please select a lead', type: 'error' });
      return;
    }
    
    const validItems = items.filter(i => i.description.trim());
    if (!validItems.length) {
      setToast({ message: 'Add at least one line item with a description', type: 'error' });
      return;
    }
  ...
  const handleSendToVendor = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedQuotation) return;
    if (!vendorForm.vendorEmail) {
      setToast({ message: 'Vendor email is required', type: 'error' });
      return;
    }
    ...
    try {
      switch (action) {
        case 'accept':
          ...
          setToast({ message: 'Quotation accepted', type: 'success' });
          break;
        case 'reject':
        ...
        case 'sendEmail':
          ...
          setToast({ message: 'Email sent to customer', type: 'success' });
          break;
        case 'finalize':
          await quotationsApi.finalize(id);
          setToast({ message: 'Quotation finalized', type: 'success' });
          break;
        case 'generatePDF':
          await quotationsApi.generatePDF(id);
          setToast({ message: 'PDF generation started', type: 'success' });
          break;
      }
      load();
    } catch (err: any) {
  ...
  const getActionButtons = (q: Quotation) => {
    ...
    if (q.status === 'Draft') {
      buttons.push(
        ...
        <button
          key="finalize"
          title="Finalize & Send to Customer"
    ...
    if (q.status === 'Sent') {
      buttons.push(
        ...
        <button
          key="vendor"
          title="Send to Vendor for Quotation"
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex gap-3 items-center">
        ...
        <button onClick={load} className="p-2 text-gray-500 hover:text-violet-600 transition-colors">
          <RefreshCw size={18} />
        </button>
      </div>
        ...
        <form onSubmit={handleCreate} className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
          ...
          <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.gstApplicable} onChange={(e) => setForm(f => ({...f, gstApplicable: e.target.checked}))} className="w-4 h-4 accent-violet-600" />
            ...
            {form.gstApplicable && (
              <div className="flex items-center gap-2">
                ...
                <input type="number" className="input-field w-20 text-sm" value={form.taxRate} onChange={(e) => setForm(f => ({...f, taxRate: Number(e.target.value)}))} min="0" max="100" />
              ...
              {items.map((item, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-start">
                  <div className="col-span-5">
                    <input className="input-field text-sm" placeholder="Description" value={item.description} onChange={(e) => updateItem(i, 'description', e.target.value)} required />
                  </div>
                  <div className="col-span-2">
                    <input type="number" className="input-field text-sm" placeholder="Qty" min={1} value={item.quantity} onChange={(e) => updateItem(i, 'quantity', Number(e.target.value))} required />
                  </div>
                  <div className="col-span-3">
                    <input type="number" className="input-field text-sm" placeholder="Unit Price" min={0} step="0.01" value={item.unitPrice} onChange={(e) => updateItem(i, 'unitPrice', Number(e.target.value))} required />
                  </div>
        ...
        <form onSubmit={handleEdit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
          ...
          <div>
            ...
            <div className="space-y-2">
              {items.map((item, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-start">
                  <div className="col-span-5"><input className="input-field text-sm" placeholder="Description" value={item.description} onChange={(e) => updateItem(i, 'description', e.target.value)} required /></div>
                  <div className="col-span-2"><input type="number" className="input-field text-sm" placeholder="Qty" min={1} value={item.quantity} onChange={(e) => updateItem(i, 'quantity', Number(e.target.value))} required /></div>
                  <div className="col-span-3"><input type="number" className="input-field text-sm" placeholder="Unit Price" min={0} step="0.01" value={item.unitPrice} onChange={(e) => updateItem(i, 'unitPrice', Number(e.target.value))} required /></div>
                  <div className="col-span-2 flex items-center gap-1">
                    <span className="text-sm font-medium text-gray-700 flex-1">{formatCurrency(item.total)}</span>
          ...
          <div><label className="label">Terms & Conditions</label><textarea rows={2} className="input-field" value={form.terms} onChange={(e) => setForm(f => ({...f, terms: e.target.value}))} /></div>
          ...
          <div className="flex gap-3 justify-end pt-2">
            <button type="button" onClick={() => { setShowEditModal(false); setSelectedQuotation(null); }} className="btn-secondary">Cancel</button>
        ...
        <form onSubmit={handleSendToVendor} className="space-y-4">
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
            <p className="text-sm text-purple-800 mb-3">
              Send this quotation request to the vendor for pricing.
            </p>
            <div className="mb-3">
              <label className="label">Vendor Email *</label>
              <input type="email" className="input-field" value={vendorForm.vendorEmail} onChange={(e) => setVendorForm(f => ({...f, vendorEmail: e.target.value}))} required placeholder="vendor@example.com" />
            ...
            <div>
              ...
              <input type="number" className="input-field" value={vendorForm.finalAmount} onChange={(e) => setVendorForm(f => ({...f, finalAmount: Number(e.target.value)}))} min="0" step="0.01" />
      ...
      <Modal isOpen={showViewModal} onClose={() => { setShowViewModal(false); setSelectedQuotation(null); }} title={`Quotation Details - ${selectedQuotation?.quotationNumber}`} size="lg">
        {selectedQuotation && (
          <div className="space-y-4 max-h-[70vh] overflow-y-auto">
            <div className="grid grid-cols-2 gap-4 p-3 bg-gray-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-gray-700">{(selectedQuotation.leadId as Lead)?.contactName || (selectedQuotation.leadId as Lead)?.contactPersonName}</p>
              </div>
              <div>
            ...
            <div>
              ...
              <table className="w-full text-sm">
                ...
                <tfoot className="border-t">
                  <tr><td colSpan={3} className="px-3 py-2 text-right font-medium">Subtotal:</td><td className="px-3 py-2 text-right">{formatCurrency(selectedQuotation.subtotal)}</td></tr>
                  {selectedQuotation.gstApplicable && (
                    <tr><td colSpan={3} className="px-3 py-2 text-right font-medium">GST ({selectedQuotation.taxRate}%):</td><td className="px-3 py-2 text-right">{formatCurrency(selectedQuotation.taxAmount)}</td></tr>
                  )}
