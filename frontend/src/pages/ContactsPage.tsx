...
import type { Contact, ContactType, Account, User } from '@/types';

// ─── Tab config ────────────────────────────────────────────────────────────────

const TABS: { key: ContactType | 'ALL'; label: string; color: string }[] = [
  { key: 'ALL',      label: 'All Contacts',       color: 'bg-gray-100 text-gray-700' },
  { key: 'TELLED',   label: 'Telled Contacts',    color: 'bg-violet-100 text-violet-700' },
  { key: 'ARK',      label: 'ARK Contacts',       color: 'bg-blue-100 text-blue-700' },
  { key: 'CUSTOMER', label: 'Customer Contacts',  color: 'bg-emerald-100 text-emerald-700' },
];
...
function canEdit(contact: Contact, user: User | null): boolean {
  ...
  // sales / engineer: only own contacts
...
function canCreate(user: User | null): boolean {
  return !!user;
}
...
export default function ContactsPage() {
  ...
  // Accounts for CUSTOMER contact dropdown
  ...
  // Load accounts when form is opened (for customer contact linking)
  ...
  const openCreate = () => {
    // HR defaults to TELLED; others get TELLED as default too
  ...
  const handleFormChange = (
    ...
    if (name === 'contactType' && value !== 'CUSTOMER') {
      setForm((prev) => ({ ...prev, [name]: value as ContactType, linkedAccountId: '' }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // HR cannot create non-TELLED contacts
    ...
    try {
      ...
      if (form.contactType === 'CUSTOMER' && form.linkedAccountId) {
        payload.linkedAccountId = form.linkedAccountId;
      }
  ...
  // ── Allowed contact types for form (HR only TELLED) ──────────────────────────
