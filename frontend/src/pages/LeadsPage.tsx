...
export default function LeadsPage() {
  ...
  const handleSendDrf = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDrfError(err?.response?.data?.message || 'Failed to send DRF email');
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <select className="input-field py-1.5 text-sm" value={form.source} onChange={(e) => f('source', e.target.value)}>
                ...
                {['Cold Call','Email','Referral','Website','Exhibition','LinkedIn','Ansys','Other'].map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            {user?.role === 'admin' && (
              <div className="col-span-2">
